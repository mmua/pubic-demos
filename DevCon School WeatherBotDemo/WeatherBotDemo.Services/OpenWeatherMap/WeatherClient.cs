...
namespace WeatherBotDemo.Services.OpenWeatherMap
{
    public class WeatherClient
    {
        ...
        {
            var res = await cli.GetStringAsync($"http://api.openweathermap.org/data/2.5/forecast/daily?q={HttpUtility.UrlDecode(city)}&mode=json&units=metric&cnt=7&APPID={AppID}");
            ...
            foreach (var z in x.list)
            {
                ...
                {
                    City = (x?.city?.name ?? "Unknown") + ", " + (x?.city?.country ?? "Unknown"),
