...
namespace UWPKiosk.Services
{
    ...
    public class CognitiveSpeechSynthesizer: ISpeechSynthesizer
    {
        ...
        public CognitiveSpeechSynthesizer(string clientId, string secret)
        {
            _auth = new Authenticator(clientId, secret);
        }
        ...
        public async Task<Stream> SythesizeAsync(string ssmlFragment, string language = "en-US", Gender gender = Gender.Female)
        {
            ...
            var voice = VoiceInfo.AllVoices.Where(v => v.Language == language && v.Gender == gender).FirstOrDefault();
            ...
            try
            {
                ...
                return await synthesize.SpeakAsync(CancellationToken.None);
            }
            catch (Exception)
            {
                return null;
    ...
    public class VoiceInfo
    {
        ...
        public static List<VoiceInfo> AllVoices => new List<VoiceInfo>
        {
            new VoiceInfo { Name = "Microsoft Server Speech Text to Speech Voice (en-US, ZiraRUS)", Gender = Gender.Female, Language = "en-US" },
            new VoiceInfo { Name = "Microsoft Server Speech Text to Speech Voice (ru-RU, Irina, Apollo)", Gender = Gender.Female, Language = "ru-RU" }
        };
    }
    ...
    public class Authenticator
    {
        ...
        public Authenticator(string clientId, string clientSecret)
        {
            ...
            this.request = string.Format("grant_type=client_credentials&client_id={0}&client_secret={1}&scope={2}",
                                          ...
                                          Uri.EscapeDataString("https://speech.platform.bing.com"));

            // renew the token every specfied minutes
            accessTokenRenewer = new Timer(new TimerCallback(async s => await OnTokenExpiredCallbackAsync(s)),
                                           this,
        ...
        private async Task OnTokenExpiredCallbackAsync(object stateInfo)
        {
            try
            {
        ...
        private async Task<AccessTokenInfo> HttpPostAsync(string accessUri, string requestDetails)
        {
    ...
    public class Synthesize
    {
        ...
        /// The ssml template
        /// </summary>
        private const string SsmlTemplate = "<speak version='1.0' xml:lang='en-us'><voice xml:lang='{0}' xml:gender='{1}' xml:name='{2}'>{3}</voice></speak>";
        ...
        public async Task<Stream> SpeakAsync(CancellationToken cancellationToken)
        {
